...
const PropertyForm = () => {
    ...
    if (id) {
      const fetchProperty = async () => {
        try {
          ...
          setError('Failed to load property data: ' + (err.message || 'Unknown error'));
        }
      };
      fetchProperty();
    }
  ...
  const InputField = ({ label, value, onChange, name, section = null, type = "text", required = false, ...props }) => (
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-5xl mx-auto px-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {bankDetailsFields.map(({ key, label }) => (
              <InputField
                key={key}
                label={label}
                name={key}
